...
public static class Native
{
    ...
    // Register Hotkey: Defines a system-wide hot key.
    [DllImport("user32.dll", SetLastError = true)]
    public static extern bool RegisterHotKey(
        IntPtr hWnd,             // Handle to the window that will receive WM_HOTKEY messages
        int id,                  // Identifier of the hot key
        uint fsModifiers,        // Key modifiers (MOD_ALT, MOD_CONTROL, MOD_SHIFT, MOD_WIN)
        uint vk);                // Virtual-key code of the hot key

    // Unregister Hotkey: Frees a hot key previously registered by the calling thread.
    [DllImport("user32.dll", SetLastError = true)]
    public static extern bool UnregisterHotKey(
        IntPtr hWnd,             // Handle to the window associated with the hot key
        int id);                 // Identifier of the hot key to be freed
    ...
    // Hotkey Modifiers (used with RegisterHotKey)
    public const uint MOD_ALT = 0x0001;
    public const uint MOD_CONTROL = 0x0002;
    public const uint MOD_SHIFT = 0x0004;
    public const uint MOD_WIN = 0x0008;
    public const uint MOD_NOREPEAT = 0x4000; // Prevents the hotkey message from being generated repeatedly when the key is held down.
...
public class DarkComboBox : ComboBox
{
		...
		private Color BackgroundColor = Color.FromArgb(45, 45, 48);     // Dark grey background
		private Color TextColor = Color.FromArgb(241, 241, 241);   // Light grey/white text
		private Color BorderColor = Color.FromArgb(67, 67, 70);     // Slightly lighter grey border
		private Color ArrowColor = Color.FromArgb(241, 241, 241);    // White arrow
		private Color SelectedItemBackColor = Color.FromArgb(0, 122, 204); // Blue highlight for selected item background
		private Color SelectedItemForeColor = Color.White;           // White text for selected item
		private Color DropDownBackColor = Color.FromArgb(37, 37, 38);      // Background color of the dropdown list
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			if (this.DropDownStyle == ComboBoxStyle.DropDownList || this.DropDownStyle == ComboBoxStyle.DropDown)
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					...
					Rectangle textRect = new Rectangle(bounds.Left + 3, bounds.Top + 2, bounds.Width - 23, bounds.Height - 4);
					TextFormatFlags flags = TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis; // Center vertically, add ellipsis if too long
			...
			Rectangle buttonRect = new Rectangle(bounds.Width - 20, bounds.Top, 20, bounds.Height);
			...
			// Draw a separator line before the button
			using (Pen sepPen = new Pen(this.Enabled ? BorderColor : SystemColors.ControlDarkDark))
			{
				g.DrawLine(sepPen, buttonRect.Left -1, bounds.Top, buttonRect.Left -1, bounds.Bottom);
			}
		...
		protected override void WndProc(ref Message m)
		{
			...
			else if (m.Msg == WM_ERASEBKGND) // Corrected 'elseif' to 'else if'
			{
		...
		protected override void OnDrawItem(DrawItemEventArgs e)
		{
			...
			TextFormatFlags flags = TextFormatFlags.Left | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis; // Align left, center vertically
			Rectangle textBounds = new Rectangle(bounds.Left + 3, bounds.Top, bounds.Width - 6, bounds.Height); // Add padding
			...
			if ((e.State & DrawItemState.Focus) == DrawItemState.Focus && (e.State & DrawItemState.NoFocusRect) == 0)
			{
				...
				using (Pen focusPen = new Pen(SelectedItemBackColor, 1)) // Use selection color for focus border
				{
					...
					g.DrawRectangle(focusPen, bounds.Left, bounds.Top, bounds.Width - 1, bounds.Height - 1);
				}
			}
...
/// <summary>
/// Inherits from NativeWindow to intercept Windows messages for a specific form,
/// specifically for handling hotkeys registered via RegisterHotKey.
/// </summary>
public class HotkeyHandler : NativeWindow
{
		// Properties to hold context
		public Form TargetForm { get; set; }
		public int HotkeyId { get; set; }
		public ScriptBlock Action { get; set; } // PowerShell ScriptBlock

		// Constructor (optional, properties can be set after creation)
		public HotkeyHandler(Form form, int hotkeyId, ScriptBlock action)
		{
			this.TargetForm = form;
			this.HotkeyId = hotkeyId;
			this.Action = action;
			// Assign the handle of the form we want to listen to
			// Note: This should ideally be done *after* the form handle is created.
			// It's often better to call AssignHandle externally after form is shown.
		...
		// Default constructor
		public HotkeyHandler() { } // Added missing body

		// Override WndProc to handle messages for the assigned window handle
		protected override void WndProc(ref Message m)
		{
			// Check if the message is WM_HOTKEY and if the WParam matches our registered ID
			if (m.Msg == Native.WM_HOTKEY && m.WParam.ToInt32() == this.HotkeyId)
			{
				// Execute the associated PowerShell action
				if (this.Action != null)
				{
					try
					{
						// Use Invoke() method of the ScriptBlock
						this.Action.Invoke();
					}
					catch (Exception ex)
					{
						// Log error - Consider a more robust logging mechanism
						Console.Error.WriteLine("FTOOL: Error executing hotkey action: " + ex.Message);
					}
				}
			...
			{
				// Handle other messages normally by calling the base class
				base.WndProc(ref m);
			}
		}
} // End of HotkeyHandler class
