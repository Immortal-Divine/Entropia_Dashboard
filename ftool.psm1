	...
	foreach ($oldId in $pausedKeys)
	{
		try
		{
			...
			foreach ($ok in $owners)
			{
				...
				$delegate = [System.Action]({ InvokeFtoolAction -Action $actionToRun }.GetNewClosure())
				...
				try
				{
					...
					{
						Write-Warning ("FTOOL: ResumeAllHotkeys failed owner '{0}': {1}" -f $ok, $_.Exception.Message)
					}
...
function InvokeFtoolAction
{
	...
	{
		Write-Verbose ('FTOOL-DEBUG: InvokeFtoolAction: Exception invoking action: {0}' -f $_.Exception.Message)
	}
}
...
function Show-KeyCaptureDialog
{
 ...
 finally
	{
		...
		{
			Write-Verbose ('FTOOL: ResumeAllHotkeys (or ResumePausedKeys) failed: {0}' -f $_.Exception.Message)
		}
		finally
		{
